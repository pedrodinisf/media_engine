...
| ~~**B-004**~~ | ~~p1~~ | ~~Run panel: `Temperature: 0,2` rendered with a comma decimal separator.~~ | Closed; see "Recently closed". | — | New `FloatInput.svelte` keeps a local text buffer seeded from `String(value)` (locale-independent) and switches to `type="text" inputmode="decimal"`. Commas accepted as a decimal separator on the way in; in-progress edits like `"0."` are detected and deferred so controlled-input round-trips don't eat trailing dots. |
...
| ~~**B-006**~~ | ~~p2~~ | ~~Run panel pre-populates `Model: gemini-2.5-flash` as the default.~~ | Closed; see "Recently closed". | — | Applied the audio-ops `json_schema_extra={"enum": …}` pattern to the four intelligence ops (extract / summarize / classify / analyze). Catalog lives at `media_engine/ops/intelligence/_models.py`. Default model unchanged (`gemini-2.5-flash` is still GA per live API probe May 2026); the win is the dropdown affordance + future-proofing for model rotation. |
| ~~**B-007**~~ | ~~p1~~ | ~~Composite ops with backend routers don't propagate `--backend` overrides.~~ | Closed; see "Recently closed". | — | Two layered fixes: (1) `Engine._resolve_backend` now preserves the operator's explicit `requested` for embedded ops so `ctx.backend` flows into composites; (2) added explicit `extract_backend: str | None` param to Summarize/Classify/Analyze, mirroring `transcribe_diarized`'s pattern. Precedence in run(): explicit composite param > ctx.backend > delegate's model-prefix router. |
| ~~**B-008**~~ | ~~p1~~ | ~~`frames.analyze` / `video.multimodal` routers leave model param unchanged when `--backend` overrides routing.~~ | Closed; see "Recently closed". | — | `Engine._resolve_backend` and `POST /run/preview` now hard-fail when the operator's `--backend` disagrees with what `op.select_backend(params)` would have routed to. Error message names both candidates and tells the operator how to fix it (change model or drop backend override). |
| ~~**B-009**~~ | ~~p2~~ | ~~Composites report `embedded ok` in doctor when delegates are unavailable.~~ | Closed; see "Recently closed". | — | `doctor.check_op` now recurses into `delegates_to` for embedded composites. Composite overall = worst of its delegates' overalls. New `OpDoctorReport.delegate_overalls` field carries the breakdown; the Settings → Doctor expand row renders it, and the Quick-fix banner picks composites up naturally. Cycle guard defends against typo'd cyclic graphs. |
...
|----|-----|----------------|
| **B-009** | `med doctor --op audio.transcribe_diarized` reported `status: ok` despite mlx-whisper missing, because the doctor never walked `delegates_to`. Recursive `check_op` now propagates the worst leaf overall up to the composite; new `delegate_overalls` field carries the breakdown; Settings → Doctor renders it inside the expand row. Cycle-guarded. | `feat(runtime/api/web): doctor walks delegates_to for composites (B-009 p2)` |
| **B-008** | `med run frames.analyze --backend vllm-mlx` with default `model=gemini-2.5-pro` silently dispatched vllm-mlx to load a gemini model. Engine and `/run/preview` now hard-fail at submit time with a message that names both backend candidates. | `fix(runtime/api): router model/backend consistency check (B-008 p1)` |
| **B-007** | Composites silently dropped `--backend` overrides because `_resolve_backend` returned `(None, None)` for embedded ops, and the composite's `run()` had no per-delegate backend hook. Fixed in two layers: engine preserves `requested` for embedded ops (ctx.backend flows in), AND added explicit `extract_backend` param to Summarize/Classify/Analyze. Precedence: explicit param > ctx.backend > delegate's router. | `fix(ops/intelligence): composites forward backend overrides to delegates (B-007 p1)` |
| **B-006** | `gemini-2.5-flash` as the Run-panel default lacked a discoverable affordance and would silently bit-rot when Google retired the id. Surface a curated `INTELLIGENCE_MODELS` enum via `json_schema_extra` (mirroring the audio-ops dropdown pattern) so SchemaForm renders a `<select>`. Model id stays `str` (not `Literal`) — CLI off-list power-user calls still validate. | `feat(ops/intelligence): curated model dropdown via json_schema_extra (B-006 p2)` |
| **B-005** | Run panel cost preview rendered `backend: —` for composite ops. Added `embedded: bool` to `/run/preview` response so the UI distinguishes "no backend resolved" from "composite, chosen at run time." | (see prior phase) |
| **B-004** | Run panel `Temperature` field rendered as `0,2` on pt-PT locale because `<input type="number">` honors the OS decimal separator. New `FloatInput.svelte` keeps a local period-decimal text buffer (locale-independent via `String(value)`) and switches the DOM input to `type="text" inputmode="decimal"`. Commas-on-input still accepted for EU keyboards; in-progress states like `"0."` are detected and don't trigger a parent re-sync. | `fix(web): locale-safe float input for SchemaForm (B-004 p1)` |
