...
def test_transcribe_schema_emits_whisper_enum() -> None:
    ...
    assert tuple(enum) == WHISPER_MODELS


def test_detect_language_schema_emits_whisper_enum() -> None:
    schema = DetectLanguageParams.model_json_schema()
    assert tuple(schema["properties"]["model"]["enum"]) == WHISPER_MODELS
...
def test_transcribe_diarized_emits_both_enums() -> None:
    ...
    assert tuple(schema["properties"]["transcribe_model"]["enum"]) == WHISPER_MODELS
